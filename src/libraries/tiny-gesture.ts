...
export default class TinyGesture<Element extends HTMLElement = HTMLElement> {
	...
	private _onTouchStart: (typeof this)["onTouchStart"] =
		this.onTouchStart.bind(this);
	private _onTouchMove: (typeof this)["onTouchMove"] =
	...
	onTouchStart(event: MouseEvent | TouchEvent) {
		...
		if (event.type !== "mousedown") {
			...
			if (
				((didTouch1 && (event as TouchEvent).changedTouches.length > 1) ||
					!didTouch1) &&
				!this.touch2
			) {
				this.touch2 =
					[...(event as TouchEvent).changedTouches].find(
						(touch) => touch.identifier !== this.touch1?.identifier,
					) || null;
				this.originalDistance = Math.sqrt(
					((this.touch2?.screenX ?? 0) -
						(this.touchMove1?.screenX ?? this.touch1?.screenX ?? 0)) **
						2 +
						((this.touch2?.screenY ?? 0) -
							(this.touchMove1?.screenY ?? this.touch1?.screenY ?? 0)) **
							2,
				);
				this.originalAngle =
					Math.atan2(
		...
		if (didTouch1 || event.type === "mousedown") {
			...
			this.touchStartX =
				event.type === "mousedown"
					...
					: this.touch1?.screenX || 0;
			this.touchStartY =
				event.type === "mousedown"
					? (event as MouseEvent).screenY
					: this.touch1?.screenY || 0;
	...
	onTouchMove(event: MouseEvent | TouchEvent) {
		...
		if (event.type !== "mousemove") {
			touch1 = [...(event as TouchEvent).changedTouches].find(
				(touch) => touch.identifier === this.touch1?.identifier,
			);
			this.touchMove1 = touch1 || this.touchMove1;
			touch2 = [...(event as TouchEvent).changedTouches].find(
		...
		if (event.type === "mousemove" || touch1) {
			const touchMoveX =
				(event.type === "mousemove"
					? (event as MouseEvent).screenX
					: touch1?.screenX ?? 0) - (this.touchStartX ?? 0);
			this.velocityX = touchMoveX - (this.touchMoveX ?? 0);
			this.touchMoveX = touchMoveX;
			const touchMoveY =
				(event.type === "mousemove"
					? (event as MouseEvent).screenY
					: touch1?.screenY ?? 0) - (this.touchStartY ?? 0);
		...
		if (
			...
			this.newDistance = Math.sqrt(
				(this.touchMove2.screenX - this.touchMove1.screenX) ** 2 +
					(this.touchMove2.screenY - this.touchMove1.screenY) ** 2,
			);
	...
	onTouchEnd(event: MouseEvent | TouchEvent) {
		...
		if (event.type !== "mouseup") {
			...
			if (
				![...(event as TouchEvent).touches].find(
					(touch) => touch.identifier === this.touch1?.identifier,
				)
			) {
				this.touch1 = null;
			...
			if (
				...
				if (this.swipedVertical) {
					if (y < 0) {
						// Upward swipe.
						if (
							(this.velocityY ?? 0) < -this.opts.velocityThreshold ||
